...
def test_effect_runtime_grit_triggers_on_signal_peak() -> None:
    signals = _signals_with_stem_key("drums", "onset_strength", [0.0, 1.0, 0.0])
    layers = {
            "layer_1": _layer("drums", effects={"grit": {"onset": 100}}),
        }
    runtime = EffectRuntime()
    baseline = runtime.tick(layers, signals, 0.0)
    triggered = runtime.tick(layers, signals, 0.01)
...
def test_effect_runtime_grit_zero_depth_is_noop() -> None:
    signals = _signals_with_stem_key("drums", "onset_strength", [0.0, 1.0, 0.0])
    layers = {"layer_1": _layer("drums", effects={"grit": {"onset": 0}})}
    runtime = EffectRuntime()
    mods = runtime.tick(layers, signals, 0.01)
...
def test_effect_runtime_grit_per_stem(
    ...
    layers = {
            slot: _layer(stem, effects={"grit": {driver_slug: 100}}),
        }
    runtime = EffectRuntime()
    baseline = runtime.tick(layers, signals, 0.0)
    modulated = runtime.tick(layers, signals, 0.01)
...
def test_effect_runtime_grit_all_stems() -> None:
    ...
    layers = {
            "layer_1": _layer("drums", effects={"grit": {"onset": 100}}),
            "layer_2": _layer("bass", effects={"grit": {"sub_bass": 100}}),
            "layer_3": _layer("vocals", effects={"grit": {"rms": 100}}),
            "layer_4": _layer("other", effects={"grit": {"centroid": 100}}),
        }
    runtime = EffectRuntime()
    mods = runtime.tick(layers, signals, 0.01)
