...
from cleave.pcm_io import SAMPLE_RATE_HZ, load_mix_pcm, load_wav_pcm_44k
...
def test_load_wav_pcm_44k_from_mono(tmp_path: Path) -> None:
...
def test_load_wav_pcm_44k_stereo_interleaved(tmp_path: Path) -> None:
    path = tmp_path / "stereo.wav"
    stereo = np.array([[1.0, 3.0], [0.0, 2.0]], dtype=np.float32)
    _write_wav(path, stereo, SAMPLE_RATE_HZ)
    out, channels = load_wav_pcm_44k(path)
    assert channels == 2
    np.testing.assert_array_equal(out, np.array([1.0, 3.0, 0.0, 2.0], dtype=np.float32))
