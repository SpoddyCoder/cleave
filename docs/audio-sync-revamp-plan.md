...
- The decode or queue position, ahead of what is actually audible by the full output latency `L` (SDL buffer plus OS mixer plus Bluetooth codec latency).
...
- Live audible clock: the estimate of what the user is hearing, used for the playhead, live cue comparison, and tap capture. Corrected by measurable buffer latency (Phase 2) plus an optional machine-local residual latency (Phase 5). Beat snap (Phase 3) snaps that capture onto the offline grid when enabled.
...
- Phase 5 residual latency, when non-zero, is factored into new placements and live monitoring. Changing it never moves already-saved markers or cues. Render never applies it.
...
- Known limit: OS mixer, Bluetooth codec, and Bluetooth input latency are not reportable in software, so on wireless setups a free-form tap can still carry residual error. Phase 3 removes that residual when snap is on. Phase 5 lets the user dial in the remainder for free-form placement and live monitoring.
...
## Phase 5 - Machine-local residual latency and measure latency

Optional compensation for unmeasurable residual latency (Bluetooth headphones, OS mixer, Bluetooth keyboard, and similar). It adjusts the **live audible clock** used for playhead, live cue comparison, and new marker/cue capture. It is not a per-cue or per-project field.
...
- Apply the offset in the audible clock:
  - **Write:** new song markers and timeline cues capture `audible_position` (buffer latency plus residual latency), then optional snap. What is stored is a plain file time.
...
- Measure latency sets this live offset only; it must not batch-shift existing markers or cues.
...
- Free-form takes recorded under the wrong latency stay wrong until re-tapped; fixing the knob later does not heal history.
- Mixing free-form takes under different latency values in one project yields inconsistent file times; render will honour that inconsistency.
- Snap-on placements are usually unaffected by residual latency; prefer snap when the beat grid is trusted.
- Another machine needs its own latency for monitoring and new taps; already-saved file times remain valid for render everywhere.

### Measure latency

Cleave plays a fixed 140 BPM metronome only (song and visuals pause): a loud downbeat on beat 1 of each bar and quieter quarter-note clicks on beats 2 to 4. The user taps Space on each bar beat (beat 1); other panels hide while detection runs. A centered progress panel shows streak (0 to 4), spread, and estimate. When four consecutive bar-beat tap deltas are consistent (within 30 ms), calibration pauses, panels restore, and a confirm modal proposes the detected latency. Apply persists the offset; Cancel exits without changing it. Esc cancels during tapping. No song beat grid is required. Manual override remains available for users who know their device.
...
- Residual latency units: offset is included in the audible clock for display and new capture; changing it does not mutate existing saved times; render paths ignore it; measure-latency inference from tap timestamps.
...
- Snap toggle and residual-latency UI and config: [cleave/config_schema.py](../cleave/config_schema.py), [cleave/viz/session.py](../cleave/viz/session.py), [cleave/viz/row_fields.py](../cleave/viz/row_fields.py), [cleave/viz/row_sections.py](../cleave/viz/row_sections.py), [cleave/viz/tuning_view_state.py](../cleave/viz/tuning_view_state.py), [cleave/viz/timeline_snap_controls.py](../cleave/viz/timeline_snap_controls.py), machine-local `editor.residual_latency_ms` in `~/.config/cleave/config.yaml`
...
- Changing the residual latency does not migrate old free-form times; recovery is re-record or replace.
...
Machine-local residual latency persists in `~/.config/cleave/config.yaml` under `editor.residual_latency_ms` (default 0).
