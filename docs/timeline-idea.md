...
Output priority: main-panel solo (if set) beats recording rules, then paused monitor preview, then timeline manual override, then committed timeline eval.
...
| `r` | Start / stop record. Start clears override, applies WYSIWYG for armed stems (see below), clears preview, and unpauses if paused. Stop leaves playback running. |
