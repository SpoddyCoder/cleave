...
[compose_pattern_mask_timeline](../cleave/timeline_presets/pattern_mask_arrange.py) takes `transition_duration`. Apply enables the mask (`transition = 1.0`) first, then passes that duration in. For add-then-remove, `t_remove - t_add` must be at least `transition_duration` plus one beat; if the section cannot fit that, the existing simultaneous-swap fallback runs. Isolated add-only and remove-only gaps are unchanged. Recast is not used.
