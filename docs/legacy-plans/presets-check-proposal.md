...
Manual preset browse (`preset_switching` `none`, or `projectm` with `preset_switching_rotation_set` `user_defined`) forces a clean black boot so presets can be judged without feedback carry-over from the previous preset. Auto `projectm` + `directory` rotation rebuilds the playlist on browse.
